                ...
                for range_start in range(0, self.batch_size, self.minibatch_size):
                    ...
                    if is_regression:
                        ...
                        with torch.autocast(
                            ...
                            current_quantiles = quantiles.gather(2, action_idx).squeeze(
                                -1
                            )
                            ...
                            with torch.no_grad():
                                ...
                                Z_tau = quantiles_tau.gather(2, action_idx_tau).squeeze(
                                    -1
                                )

                            Z_tau_hat = current_quantiles.detach()
                            gradients_tau = (
                                2 * Z_tau - Z_tau_hat[:, :-1] - Z_tau_hat[:, 1:]
                            )
